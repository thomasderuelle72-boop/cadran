...
- **Trois palettes, deux modes, deux axes indépendants** — *Cadran* (vert profond et cuivre sur papier crème), *Registre* (encre bleue et oxblood, angles vifs, filets marqués) et *Ardoise* (gris froid et indigo, formes adoucies), chacune en clair et en sombre. Les croiser en un seul réglage aurait donné six valeurs à tenir en correspondance ; séparés, on change de famille de couleurs sans perdre son mode. Le réglage « système » et la palette par défaut ne posent aucun attribut, ce qui laisse une page chargée sans JavaScript correctement habillée.
- **Une palette change aussi la géométrie** — rayon des cartes, force des filets, ombres. Un thème qui ne fait que recolorer se reconnaît immédiatement pour ce qu'il est : une variante. Registre pince les angles à 4 px comme un livre-journal, Ardoise les arrondit à 16 px.
- **Les couples de couleurs de courbes sont validés, pas choisis** — bande de clarté, plancher de chroma, séparation en vision déficiente, contraste sur le fond. Registre sépare ses deux séries de ΔE 18,9 en vision protanope et Ardoise de ΔE 29,5, contre 7,2 pour le couple historique de Cadran, qui reste au plancher et impose donc les étiquettes directes en bout de courbe.
- **Les graphiques suivent les deux axes** : Recharts peint en SVG avec des attributs `stroke` et `fill` qu'une classe utilitaire n'atteint pas, donc un hook relit les tokens à chaque changement de mode *et* de palette.
